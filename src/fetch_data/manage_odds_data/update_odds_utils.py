...
# API_KEY = "e86efcc924msh1f4a52ac1d3c1dep1e23f9jsn687364ab3e47"
...
def process_odds_date(date: str, BASE_URL:str, HEADERS:dict, is_today = False) -> pd.DataFrame:
    sport_id = 4
    ...
    spread_home_field = 'point_spread_home_delta' if not is_today else 'point_spread_home'
    spread_away_field = 'point_spread_away_delta' if not is_today else 'point_spread_away'
    ...
    for event in matches:
        ...
        match_data.append({
            ...
            'average_total_line': sum(total_lines) / len(total_lines) if total_lines else None,
            'most_common_moneyline_home': most_common(moneyline_home),
            'average_moneyline_home': sum(moneyline_home) / len(moneyline_home) if moneyline_home else None,
            'most_common_moneyline_away': most_common(moneyline_away),
            'average_moneyline_away': sum(moneyline_away) / len(moneyline_away) if moneyline_away else None,
            'most_common_spread_home': most_common(spread_home),
            'average_spread_home': sum(spread_home) / len(spread_home) if spread_home else None,
            'most_common_spread_away': most_common(spread_away),
            'average_spread_away': sum(spread_away) / len(spread_away) if spread_away else None,
        })
...
def process_odds_df(df_odds):
    ...
    df_odds.loc[mask, 'game_date_adjusted'] = df_odds.loc[mask, 'game_date_adjusted'] - pd.Timedelta(days=1)
    df_odds['game_date'] = pd.to_datetime(df_odds['game_date']).dt.strftime('%Y-%m-%d')
    df_odds['game_date_adjusted'] = pd.to_datetime(df_odds['game_date_adjusted']).dt.strftime('%Y-%m-%d')
    ...
    df_odds_renamed = df_odds.rename(columns={
    ...
    final_columns = [
        "GAME_DATE", "TEAM_NAME_TEAM_HOME", "TEAM_NAME_TEAM_AWAY", 
        "MONEYLINE_HOME", "MONEYLINE_AWAY",
        "TOTAL_OVER_UNDER_LINE", "SPREAD_HOME"    
    ]
...
def merge_teams_df_with_odds(df_odds, df_team):
    ...
    df_team['TEAM_NAME'] = df_team['TEAM_NAME'].map(lambda x: TEAM_NAME_EQUIVALENT_DICT.get(x, x))
    ...
    dupes_mask = df_odds.duplicated(subset=['GAME_DATE', 'TEAM_NAME_TEAM_HOME'], keep=False)
    ...
    dupes_mask = df_odds.duplicated(subset=['GAME_DATE', 'TEAM_NAME_TEAM_AWAY'], keep=False)
    ...
    df_home = df_odds[['GAME_DATE', 'TEAM_NAME_TEAM_HOME', 
                        'MONEYLINE_HOME', 'TOTAL_OVER_UNDER_LINE', 'SPREAD_HOME', 
                        'GAME_DATE_ORIGINAL']].copy()
    ...
    df_away = df_odds[['GAME_DATE', 'TEAM_NAME_TEAM_AWAY', 
                        'MONEYLINE_AWAY', 'TOTAL_OVER_UNDER_LINE', 'SPREAD_AWAY', 
                        'GAME_DATE_ORIGINAL']].copy()
    ...
    df_missing_tot_merged = (
        df_missing_tot_merged
        .dropna(subset=['TOTAL_OVER_UNDER_LINE_inv'])  # keep rows that actually have TOT
        .drop_duplicates(subset=['GAME_DATE', 'TEAM_NAME'], keep='first')
        .copy()
    )

    # 4) Rename so we only keep one TOT column
    df_missing_tot_merged['TOTAL_OVER_UNDER_LINE_new'] = df_missing_tot_merged['TOTAL_OVER_UNDER_LINE'].fillna(
        df_missing_tot_merged['TOTAL_OVER_UNDER_LINE_inv']
    )
    ...
    df_team_final['TOTAL_OVER_UNDER_LINE'] = df_team_final['TOTAL_OVER_UNDER_LINE'].fillna(
        df_team_final['TOTAL_OVER_UNDER_LINE_new']
    )
    ...
    #drop duplicates
    df_team_final.drop_duplicates(subset=['GAME_DATE', 'TEAM_NAME', 'HOME'], keep=False, inplace=True)

    return df_team_final

def update_odds_df(date_to_predict,odds_folder, df_name, season_to_download, ODDS_API_KEY, BASE_URL):
    HEADERS = {
    ...
    game_finder = LeagueGameFinder(season_nullable=season_to_download, league_id_nullable="00")
    games_df = game_finder.get_data_frames()[0]

    unique_dates = sorted(pd.to_datetime(games_df['GAME_DATE']).dt.strftime('%Y-%m-%d').unique(), reverse=True)
    unique_dates = [date for date in unique_dates if date not in df_odds['game_date'].dt.strftime('%Y-%m-%d').values]
    ...
    df_tomorrow = process_odds_date(tomorrow_date.strftime('%Y-%m-%d'), BASE_URL, HEADERS, is_today=True)
...
if __name__ == "__main__":
    ...
    df = update_odds_df(odds_folder, df_name, ODDS_API_KEY, BASE_URL="https://therundown-therundown-v1.p.rapidapi.com")