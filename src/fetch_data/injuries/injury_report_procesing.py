...
from .TEAM_CONVERSION_DICT import TEAM_CONVERSION_DICT
...
def get_latest_pdf(nba_injury_report_url, save_path="latest_report_today.pdf"):
    try:
        ...
        timestamps = [(link, match.group(1), match.group(2).upper()) 
                      for link in injury_reports if (match := timestamp_regex.search(link))]
        ...
        timestamps = [(link, convert_to_24h(hour, period)) for link, hour, period in timestamps]
...
def read_injury_report(pdf_path):
    ...
    col_names = ["Game Date", "Game Time", "Matchup", "Team", "Player Name", "Current Status", "Reason"]
    ...
    for page in doc:
        ...
        for block in blocks:
            ...
            if (
                len(text_elements) == 2 and
                any(status in text for status in ["Out", "Questionable", "Probable", "Doubtful", 'Available'])
                ):
            ...
            if len(text_elements) >= 3 and (";" in text_elements[-2] or "-" in text_elements[-2]):
                text_elements[-2] = f"{text_elements[-2]} {text_elements[-1]}"  # Merge last two elements
                text_elements.pop()

            
            if len(text_elements) >= 4 and (";" in text_elements[-3] or "-" in text_elements[-3]):
                text_elements[-3] = f"{text_elements[-3]} {text_elements[-2]} {text_elements[-1]}"  # Merge last three elements
                text_elements.pop()
                text_elements.pop()
            ...
            if len(current_line) != 7:
                ...
                for element in current_line:
                    for category in col_names:
                        ...
                        if last_matched_col == "Player Name" and category not in ["Current Status", "Reason"]:
                            continue
...
def process_injury_data(games: pd.DataFrame, injur_df: pd.DataFrame):
    ...
    # Create merge keys
    injur_df['merge_key'] = injur_df['Game Date'].dt.strftime('%Y%m%d') + injur_df['Matchup'].str.replace('@', '', regex=False)
    games['merge_key'] = games['GAMECODE'].str.replace('/', '', regex=False)

    # Merge to get GAME_ID
    injur_df = injur_df.merge(games[['merge_key', 'GAME_ID']], on='merge_key', how='left')

    # Convert player names from "Last, First" to "First Last"
    injur_df['Player Name'] = injur_df['Player Name'].apply(convert_name)

    # Get Player ID
    injur_df['Player ID'] = injur_df.apply(lambda row: get_player_id(row['Player Name'], row['Team']), axis=1)
