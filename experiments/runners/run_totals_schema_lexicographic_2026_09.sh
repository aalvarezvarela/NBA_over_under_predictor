#!/usr/bin/env bash
# Compare totals models across closing-data schemas 2.0, 2.1 and 2.2.
# Runs sequentially on one CUDA device. Set SKIP_EXISTING=1 to resume safely.
set -uo pipefail

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="totals_schema_lexicographic_2026_09"
...
CONFIGS=(
  "$CONFIG_DIR/a_total_points_2_0.yaml"
  "$CONFIG_DIR/b_line_error_2_0.yaml"
  "$CONFIG_DIR/c_total_points_2_1.yaml"
  "$CONFIG_DIR/d_line_error_2_1.yaml"
  "$CONFIG_DIR/e_total_points_2_2.yaml"
  "$CONFIG_DIR/f_line_error_2_2.yaml"
)
...
log "Totals schema lexicographic campaign started $(date)"
log "Runs: ${#CONFIGS[@]} sequential CUDA experiments, 150 trials each"
log "Selector: best pooled O/U win rate in top 15% pooled-MAE band, capped at +0.04 MAE"
log "Logs: $LOG_DIR"
