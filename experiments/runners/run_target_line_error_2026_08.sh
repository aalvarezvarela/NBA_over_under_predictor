#!/usr/bin/env bash
# Four line-error experiments, sequential on one CUDA machine.
# Run the total-points runner on the other machine at the same time.
#
#   bash experiments/runners/run_target_line_error_2026_08.sh
#
# Detached:
#   nohup bash experiments/runners/run_target_line_error_2026_08.sh > /dev/null 2>&1 &
#
# Resume after an interruption without repeating completed cells:
#   SKIP_EXISTING=1 bash experiments/runners/run_target_line_error_2026_08.sh
set -uo pipefail  # One failed cell must not cancel the remaining experiments.

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="target_line_error_2026_08"
