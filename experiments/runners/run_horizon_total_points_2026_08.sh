#!/usr/bin/env bash
# Sixteen experiments, sequential on one CUDA machine: fifteen total-points
# cells plus one T-360 classifier cell.
#
...
# two-way contention.
#
#   bash experiments/runners/run_horizon_total_points_2026_08.sh
#
# Detached:
#   nohup bash experiments/runners/run_horizon_total_points_2026_08.sh > /dev/null 2>&1 &
#
# Resume after an interruption without repeating completed cells:
#   SKIP_EXISTING=1 bash experiments/runners/run_horizon_total_points_2026_08.sh
set -uo pipefail  # One failed cell must not cancel the remaining experiments.

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="horizon_total_points_2026_08"
...
CONFIGS=(
  ...
  "$CONFIG_DIR/i_classifier_intermediate_t360.yaml"
