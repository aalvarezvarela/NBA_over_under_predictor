#!/usr/bin/env bash
# Run only the unweighted LINE_ERROR seven-snapshot campaign and its 6h/4h
# controls. The pooled cell uses the snapshot-aware wrapper; the controls use
# the ordinary one-row-per-game CLI.
#
# Foreground:
#   bash experiments/intermediate_line_2026_08/run_line_error_7snapshot_6h_4h.sh
#
# Detached:
#   nohup bash experiments/intermediate_line_2026_08/run_line_error_7snapshot_6h_4h.sh \
#       > /dev/null 2>&1 &
#
# Existing artifacts with the exact experiment name are skipped by default.
# Force all three to rerun with SKIP_EXISTING=0. The targeted full-data
...
CAMPAIGN="intermediate_line_7snapshot_6h_4h_2026_08"
...
NAMES=(
  "intermediate_pooled_7snapshot_line_error_no_time_decay"
  "intermediate_t360_control_line_error_no_time_decay"
  "intermediate_t240_control_line_error_no_time_decay"
  "intermediate_t720_control_line_error_no_time_decay"
)
CONFIGS=(
  "${CONFIG_DIR}/pooled_7snapshot_line_error_no_time_decay.yaml"
  "${CONFIG_DIR}/t360_control_line_error_no_time_decay.yaml"
  "${CONFIG_DIR}/t240_control_line_error_no_time_decay.yaml"
...
log " Started  : $(date)"
log " Runs     : ${#CONFIGS[@]} (1 pooled, 3 controls: 4h / 6h / 12h)"
...
for i in "${!DATASETS[@]}"; do
  ...
  if [[ ! -s "$dataset" ]]; then
    ...
    log "Build all three with:"
    log "  bash ${CONFIG_DIR}/prepare_line_error_7snapshot_6h_4h.sh"
    exit 1
  fi
  checksum="$("${PY[@]}" -c \
...
# Unlike a directory-wide preflight, passing the three YAML paths checks only
# this matched comparison group even though the configs live beside the older
# six-snapshot campaign definitions.
...
log "  1. pooled T=360 (6h) vs t360 control"
log "  2. pooled T=240 (4h) vs t240 control"
log "  3. pooled T=720 (12h) vs t720 control -- the pair the earlier"
log "     campaign disagreed on, now on matched bytes and 7 seeds"
log "  4. holdout ROI only as a smaller-sample sanity check"
