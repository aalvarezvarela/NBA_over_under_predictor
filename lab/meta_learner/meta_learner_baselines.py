    ...
    *,
    ...
    if np.any(tie_mask):
        reference_sign = np.sign(
            pd.to_numeric(df[reference_tiebreaker_col], errors="coerce").to_numpy(
                dtype=float
            )
        )
        majority[tie_mask] = reference_sign[tie_mask]

    return pd.Series(majority, index=df.index, name=output_name)
